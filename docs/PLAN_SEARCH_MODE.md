...
Статус: **РЕАЛИЗОВАНО (2026-08-26)** — 960 passed, 2 skipped. Остались ручной смоук и A/B-проверка (критерии 2-4 ниже).
...
> **Уточнение (2026-08-26, план PLAN_AGENT_DEGRADATION.md):** при
> `use_approaches=False` из контекста скрываются подходы, хинты и сессионные
> положительные факты, а инструменты `get_approaches`/`get_hints` убираются из
> списка tools — «чистый поиск» не получает подсказок памяти.
