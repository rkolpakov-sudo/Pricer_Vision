...
- Причина клик-ошибки в логе — в НАЧАЛЕ сообщения (`agent_loop._summarize_result` показывает
  error до 240 символов, не 100) — сырой Playwright-текст не скрывает, почему fallback не сработал.

### Источники цен: маркетплейсы и «доверенные цены»
- **Маркетплейсы-агрегаторы** (`market.yandex.ru` и т.п.) — НЕ источники цены магазина:
  карточки нестабильны (404), цена не от магазина. Исключены из rule-8 реюза, «Похожих цен»
  в контексте, `get_confirmed_prices`, целевых сайтов/подходов/fallback, подсказок карточек,
  сохранения (`save_confirmed_price`/`save_approach` отклоняют) и TaskScheduler
  (`_MARKETPLACE_DOMAINS`, `_marketplace_site()` в agent_loop / `_is_marketplace()` в task_scheduler).
  Правило 12 в SYSTEM_PROMPT явно запрещает market.yandex.ru.
- **`_trustable_price(p)`** — подтверждённая цена пригодна как источник только если: не маркетплейс,
  URL = карточка товара (не главная/поиск/семейная), не stale, confidence ≥ 0.5. Применяется в
  rule-8, контексте и graph-tool. Галлюцинированные записи (conf 0.5 без карточки) агент не видит.

### GUI: полный URL в таблице результатов (main.py)
- Регрессия `url[:80]` в `_on_row_done` резала URL на 80 символов → клик давал битую ссылку (404),
  терялся числовой суффикс карточки (`…/103731804`). Теперь URL (колонка 7) — ПОЛНЫЙ текст +
  tooltip + `Qt.UserRole`; двойной клик открывает колонку 7 (`item.data(Qt.UserRole) or text()`),
  а не 6 «Сайт».
...
- **Id модели из editable-комбобокса** берётся из отображаемого текста
  (`llm_providers.model_id_from_combo_text`): `currentData()` хранит данные ТЕКУЩЕГО
  индекса и при ручном вводе мог сохранять устаревшую модель (регрессия very-high/very-low).
- Стартовый лог LLM включает имя модели (`provider=… model=…`) — видно, что реально
  используется, без открытия файла конфига.
...
- **1320 тестов, 10 skipped**, 0 failures. Запуск: `python -m pytest -q`.
...
## Сбор spec_text и запись результатов (после инцидента vtk_spec_v2)

### spec_text = Наименование + «Тип, марка» + «Код оборудования»
- `ExcelWriter.build_item_name` собирает spec_text из трёх групп колонок на равных:
  «Наименование» → «Тип, марка, обозначение документа» → «Код оборудования».
  Артикул (065B8203R, 013G7027) участвует в поиске наравне с типом/маркой.
- **Поглощение дублей с нормализацией** (`_value_absorbed`): одинаковые данные из
  разных колонок попадают в запрос одним упоминанием. Значение поглощается, если
  norm(значение) ⊆ norm(накопленного) («TR 84» ⊆ «…TR84…», «ENERGOFLEX» ⊆ «Трубка
  ENERGOFLEX Super SK…») или все его слова уже есть в накопленном; короткие чистые
  числа подстрокой не поглощаются («100» ≠ часть «500x1000»). Частичное пересечение
  (имя содержит бренд LEMAX, тип несёт размер C10 500x400) — значение добавляется
  целиком.
- НЕ добавляются: чистые ссылки на стандарты (`is_standard_reference` без размера),
  пустые/дублирующие значения. **Размер внутри ссылки сохраняется**: если «Тип,марка»
  = «ГОСТ 3262-75 Ø20», то цитата «ГОСТ 3262-75» дропается, а «Ø20» остаётся
  (`standard_citation_tail`) — иначе строки Ø20..Ø50 схлопнулись бы в один spec_text
  и получили одну цену (прогон «Трубопроводы…»).
  Производитель — отдельным полем (правило 16), в запрос не конкатенируется.
- **Предохранитель уникальности spec_text**: `get_specs()`/`spec_for_row()` строятся из
  общего кэша (`_disambiguate_spec_texts`): если у строк с РАЗНЫМ сырьём
  (name_raw/spec/article/brand) текст схлопнулся в один — к дублю добавляется
  детерминированный суффикс-отличие (Ø/DN/x-размер → слово → статья). Идентичное сырьё
  (один товар × разное кол-во) легитимно делит текст (одна цена).
- spec_text = поисковый запрос агента И ключ rule-8/semantic-cache/негативного
  кэша/графа — поэтому в нём обязаны различаться все типоразмеры и артикулы.

### Строгие размеры на путях авто-реюза
- `_size_key` извлекает слэш-типоразмеры («60/40-2», «28/32», «20/20/16»).
- `product_name_matches(..., strict_sizes=True)` отклоняет совпадение, когда размер
...
### Idle-таймаут строки (не «по стенам»)
- `mcp_agent_runner._run_row_with_idle_timeout`: строка работает, пока агент жив
  (колбэки status/monitor/site_visit обновляют `activity['last']`). Отмена только
  при зависании (`run.row_idle_timeout_seconds`, по умолчанию 180) или жёстком
  пределе `run.row_max_seconds` (900). Раньше `asyncio.wait_for(timeout=300)` убивал
  строку за 15с до успеха даже при активной работе.
- Правило 15 промпта: НЕ собирать URL поиска вручную и НЕ делать percent-кодирование
  кириллицы (агент перебирал %D0%B5/%D0%BA/%D0%B8 вместо %D1%85); искать через строку
  сайта, при SPA — копировать URL из адресной строки (`location.href`); перед вводом
  нового запроса ОЧИСТИТЬ поле поиска.
- Автозачистка поля: перед `browser_type` программа очищает поле нативно через
  `_clear_field_js` (browser_evaluate, value setter + input event), если передан
  CSS-селектор. Устраняет склейку запросов на SPA («МС-140 Мх500МС-140»).
- Совет перед кликом по карточке: `_model_mismatch_hint` — если название элемента
  содержит модель, НЕ пересекающуюся с моделью спецификации (МС-40 vs МС-140, LEMAX VC
  vs МС-140), программа предупреждает «не открывай карточку для проверки» (система-
  советник: LLM решает, но видит риск).
- Защита от «мысленных циклов»: `content_only_rounds` — при N подряд раундах, где LLM
  размышляет без tool_calls и без цены, добавляется напоминание (2) и force-совет (3)
  «извлеки цену или переключись на другой сайт».
- Кэш повторов browser_evaluate: `eval_cache[(домен, хэш JS)]` — повторный evaluate с тем
  же JS на той же странице берётся из кэша (bridge не вызывается), результат проходит
  обычную обработку (StuckDetector no_change, факты) и получает подсказку «не повторяй
  извлечение».
- Расчёт цены радиатора по секциям: `src/radiator_section_pricer.py` — если точное
  количество секций (`-0,9-N`) не найдено ни на одном сайте, рассчитывается цена
  как `цена_секции × N` из известного варианта той же модели. Только для радиаторов
  (`plumbing_heating_radiators`), только после полного перебора сайтов, только на основе
...
- Рейтинг сайтов: `learning_loop.site_profiles` ключ `тип|бренд`, учёт неудач,
  порог `MIN_SAMPLES=3`, скор `success_rate*0.5 − avg_attempts/300*0.3 − block_count*0.2`;
  сайт профиля поднимается в контексте агента выше успешных подходов.
- Дополнительно `_build_context._sort_key` ранжирует сайты внутри «успешных» по
  суммарной `success_count` подходов (santech с 48 успехами стоит выше satro-paladin с 2).
- Рейтинг учитывает МОДЕЛЬ товара: `success_scores` фильтруются по
  `model_designators` — успехи LEMAX-радиаторов не поднимают satro-paladin для МС-140
  (каждый товар идёт на сайт, где его модель находили успешно).

### Защита найденной цены (нельзя уходить с сайта)
- Код-гейт в `browser_navigate`: если `price_candidate_seen` (цена распознана в
  результатах/карточке) и `price_confirmed=False` — попытка перейти на ДРУГОЙ домен
  отклоняется с сообщением «сначала сохрани цену через save_confirmed_price». Это
  устраняет потерю найденного товара (регрессия: позиция 36 — МС-140 на santech).
- `_price_is_relevant` фильтрует кандидата: цена считается найденной ТОЛЬКО если в
  контенте есть тип/артикул из spec_meta или специфичное слово спецификации
  (МС-140, чугунный); родовые слова («радиатор», «кран») недостаточны. Это защищает
  от ложного кандидата — цены чужого товара из выдачи поиска (электрический щиток
  3 555 ₽ при запросе «МС-140»), из-за которой агент застревал на сайте и сохранял
  чужой товар как «цену».

### Подходы с хеш-рефами (чистка и защита)
- Исторические подходы могли сохранить `target` как хеш-реф accessibility-дерева
  (`e80`) — недействителен между сессиями/бэкендами. Проведена миграция БД: у 339
  подходов такие target удалены (821 с role-element сохранены — переносимы).
- `_apply_approach` очищает хеш-реф target без element → `_auto_target=True`;
  `_portable_step_target` при этом возвращает '' (LLM сам найдёт элемент на странице,
  text шага ввода не подставляется как локатор).

### Матчинг h1 карточки (система-советник)
- Система НЕ решает «правильно/неправильно»: матчер строгий, а отсутствующие слова
  выносятся в advisory-СОВЕТ с аргументами, решение принимает LLM.
- `_mismatch_warning_content` разделяет отсутствующие слова на КЛЮЧЕВЫЕ (тип/материал/
  соединение/бренд — их отсутствие = другой товар) и ОПИСАТЕЛЬНЫЕ (серия/комплектация —
  сайт может опускать в сокращённом h1), и советует перепроверить НА КАРТОЧКЕ,
  а не на других сайтах/Яндексе.
- Матчер корректно парсит: транслитерацию брендов («Лемакс»≈«lemax», «Ридан»≈«ridan»)
  и тройной размер «10х500х600» (тип × высота × ширина) → «500x600» — это исправление
  парсинга, а не «решение системы».
- Правило 17: находясь в карточке с извлечённой ценой и h1 — сохранить ПЕРВЫМ делом
  (с confirm=true при расхождении только в описательных словах); уход с карточки до
  сохранения = потерянный результат.
