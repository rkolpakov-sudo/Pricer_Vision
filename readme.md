...
- **Поглощение дублей с нормализацией** (`_value_absorbed`): одинаковые данные из
  ...
  целиком.
- НЕ добавляются: ссылки на стандарты (`is_standard_reference`), пустые/дублирующие
  значения. Производитель — отдельным полем (правило 16), в запрос не конкатенируется.
