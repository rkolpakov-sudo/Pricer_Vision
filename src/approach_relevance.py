...
# Характеристика, которая может сидеть ВНУТРИ ссылки на стандарт
# («ГОСТ 3262-75 Ø20», «ГОСТ 3262-75 20х2,8»). Такие значения нельзя дропать
# целиком — иначе строки, различающиеся только размером, схлопнутся в один spec_text.
_DIM_IN_STANDARD_RE = re.compile(
    r"(?i)(Ø\s*\d|Ду\s*\d|ДУ\s*\d|DN\s*\d"
    r"|\b\d+(?:[.,]\d+)?\s*[хx×*]\s*\d+(?:[.,]\d+)?"
    r"|\b\d+\s*мм(?![\wа-яё])|\b\d+(?:[.,]\d+)?\s*м(?![\wа-яё]))"
)


def standard_citation_tail(text: str) -> str:
    """Из значения, начинающегося со ссылки на стандарт, возвращает хвост-характеристику.

    «ГОСТ 3262-75 Ø20» → «Ø20» (размер сохраняется).
    «ГОСТ 3262-75 20х2,8» → «20х2,8».
    Чистая ссылка «ГОСТ 3262-75» → '' (значение дропается, как раньше).
    Вызывать только при is_standard_reference(text) == True.
    """
    t = (text or "").strip()
    m = _DIM_IN_STANDARD_RE.search(t)
    if not m:
        return ""
