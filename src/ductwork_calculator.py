...
# ПОСЛЕ любого разделителя. Дефис допускается только если продолжается другая
# размерность: 'p/р' (круглый: p160-p100) ИЛИ цифра с 'х'/'x' (прямоугольник:
# p400-500x250). Так латинские модели вида «Насос P125-40» (после дефиса
# простая цифра без 'х') не портятся.
_P_OCR_DIAM_RE = re.compile(
    r'(?<![A-Za-zА-Яа-я0-9])[pр](\d{2,4})(?=\s|$|[хx,/;)]|(?=-[pр])|(?=-\d+[хx]))',
)
