...
### ⚠️ Нужна очистка заражённого кэша (131 цена сессии)
Сохранённые без проверки цены остаются в БД и будут переиспользованы. Сбросить подтверждённые
цены, добавленные этой сессией (id >= 1140):
```
venv\Scripts\python.exe -c "import sqlite3; c=sqlite3.connect('data/pricer.db'); c.execute('DELETE FROM confirmed_prices WHERE id >= 1140'); c.commit(); print(c.total_changes)"
```
Либо в UI «Ассистент → Цены» удалить вручную. После очистки — перезапустить прогон (теперь
несовпадающие товары отклоняются автоматически).
