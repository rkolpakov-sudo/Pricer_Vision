...
«Перезапустить отмеченные» обработал ПЕРВУЮ позицию (excel 63 КДР 200х100,
нашёл цену 1301 за 321с, invalid снят), после чего завис: вторая строка не
запускалась (между 17:22:20 bridge stopped и 17:26:50 автосейвом — тишина).
Строка статуса не показывала текущее действие.

### Причина (главная)
`_on_retry_done` вызывал `_launch_next_queued_retry()` → `_retry_single_row()`,
которая НАЧИНАЕТСЯ с guard `if self._processing_active: return`. Но
`_processing_active` оставался True (устанавливался для первой строки и не
сбрасывался перед запуском следующей) → guard мгновенно возвращался, очередь
висела. Остальные 18 позиций не обрабатывались и оставались invalid.

### Причина (статус-строка)
Retry-runner шлёт живые статусы через `status_signal`, но `_retry_runner`
status_signal нигде не подключался → строка статуса не обновлялась.

### Фикс (main.py)
1. `_on_retry_done`: сбрасывать `_processing_active = False` ПЕРЕД запуском
   следующей строки очереди (guard в _retry_single_row пропускает).
2. `_retry_single_row`: при старте строки показывать в `status_label`
   «Перезапуск отмеченных: «<товар>…» (осталось N)» / «Повтор: <товар>».
3. Подключён `status_signal` retry-runner → новый `_on_retry_status`
   (живой прогресс в строку статуса).

### Тесты
`tests/test_main.py::test_batch_queue_advances_after_row_done`: очередь из 2
строк полностью доливается (guard не блокирует после сброса флага), обе строки
запущены, invalid снят.

**23 passed** в test_main.py.

---

## 2026-09-08 — Инвалидизация 19 позиций: применена к АКТУАЛЬНОЙ сессии + гарантия сохранности через полный прогон

### Ошибка первой инвалидизации
Первый раз 7 позиций были помечены invalid в `_current.json`, НО файл был
устаревшим (сохранён 07.09 18:56, 379 результатов): прогон 08.09 10:49 записал
priced-выгрузку (447 строк с ниппелями), но `_on_all_done` НЕ вызывает
`_auto_save_session` — сессия автосохраняется только в `closeEvent`. После
закрытия окна сессия перезаписалась из памяти (424 результата, включая ниппели
421–432), и мои 7 пометок были стёрты.

### Корректная инвалидизация (после закрытия сессии)
В актуальном `_current.json` (08.09 12:25, 424 результата, excel 2..432):
помечены invalid=True все 19 скомпрометированных позиций:
- excel 63–68 (КДР воздухораспределители, duct-цены 1652–3304₽)
- excel 86 (щелевой диффузор PV, 981.43₽)
- excel 421–432 (ниппели латунные/переходные/приварные, 40.16₽)
Проверено: список отсортирован, дублей нет, found_count=318 совпадает.

Бэкап: `data/sessions/_current_backup_20260908_1225_pre_invalidation.json`.

### Гарантия сохранности через полный прогон (проверено симуляцией)
Пользователь хочет заняться 19 позициями ПОСЛЕ полного прогона. Подтверждено
по коду и симуляцией на реальной сессии:
- runner при restore делает `result = dict(_match)` (копия сохраняет invalid=True);
- для каждой из 19 есть запись в сессии → runner находит матч по excel_row →
  `restore` → `continue`: live-поиск и ductwork для них НЕ запускаются (0 в live);
- `_fill_session_gaps` не трогает присутствующие (в т.ч. invalid) позиции;
- `_merge_session_results`/`_on_all_done` сохраняют invalid;
- автосейв/загрузка (save_session/load_session) сохраняют поле invalid в JSON.

### Тесты
`tests/test_main.py`: +4 (копия restore хранит invalid; invalid переживает
restore+merge полного прогона; invalid переживает автосейв-загрузку).

**22 passed** в test_main.py.

---

## 2026-09-08 — FEAT: пометка «невалидна» + пакетный перезапуск отмеченных (вариант C)

### Идея пользователя
Пользователь сам определяет, что часть результатов невалидна (по любой причине).
Раньше единственный путь — ручной попозиционный «Повторить поиск (с нуля)».
Решение: НЕ новый механизм поиска, а **scheduler для существующего retry**: метка
«невалидна» ставится на позиции, затем кнопка «Перезапустить отмеченные» гоняет
их по одной через тот же `_retry_single_row` (fresh-поиск).

### Почему вариант C (не A)
- A менял restore-матчинг runner (`invalid` в ядре) → риск регрессий сессий/порядка.
- C: 0 правок в runner/schemas/restore/merge. Runner вообще не знает про invalid —
  каждая отмеченная строка запускается штатным fresh-поиском одной строки.

### Правки (всё в main.py)
1. **`_on_retry_row_done`**: insert → **upsert по excel_row** (замена существующей
   записи на месте; дубль исключён). Сохраняет прежний `invalid` при «не найдено»,
   снимает при найденной цене.
2. **`_retry_single_row`**: убрано удаление строки из `_restored_results` перед
   запуском (fresh=True уже отключает reuse; удаление создавало «окно потери» при
   краше между стартом и результатом).
3. **ПКМ → «Пометить невалидной (переискать)» / «Снять пометку»**: toggle
   `invalid=True` на записи в `_restored_results`. Метка — GUI-признак, runner её
   не читает; персистентна в сессии (JSON).
4. **Кнопка «⟳ Перезапустить отмеченные (N)»** (top-bar, disabled при N=0 или во
   время обработки): одно подтверждение → purge памяти отмеченных → очередь.
5. **Scheduler**: `_retry_queue` (excel_row) + `_launch_next_queued_retry()` —
   по завершении одной строки (`_on_retry_done`) запускает следующую; пусто →
   `_finish_retry_batch`. `_on_retry_error` прерывает очередь с сообщением.
6. **Визуал**: единый `_result_color` (invalid → danger) + префикс «⚠ » у цены +
   тултип во всех 4 рендерах; кнопка-счётчик обновляется.
7. **`stop_processing`**: останавливает и `_retry_runner`, чистит очередь.
8. `_retry_single_row` включает `Стоп`, выключает `Старт` на время поиска.

### Защита от прошлых регрессий
- Порядок строк: upsert по excel_row, `_sort_by_excel` не меняется.
- Восстановление сессии/gap-fill: invalid-строки остаются в списке (не удаляются) —
  дыр нет, runner не затронут.
- Остановка пакета: обработанные обновлены, необработанные остаются invalid.
- Одиночный «Повторить» работает тем же путём.

### Текущая инвалидизация (19 скомпрометированных duct-позиций)
- В `_current.json` помечены invalid: excel 63–68 (КДР), 86 (диффузор PV) = 7.
- Ниппели 421–432 в сессии отсутствуют (max excel 387) — при полном «Старт» будут
  обработаны как новые (после duct-фикса A+C+D уйдут агенту, а не в duct).
- Бэкап до правки: `data/sessions/_current_backup_20260908_pre_invalidation.json`.

### Тесты
`tests/test_main.py`: +5 (toggle invalid, кнопка при обработке, retry-upsert без
дубля, найденная цена снимает invalid, «не найдено» сохраняет invalid).

**255 passed** в затронутых файлах.

---

## 2026-09-08 — FIX: сантехника/готовые изделия ошибочно уходили в duct calculator (A+C+D)

### Симптом
В смешанной спецификации (вент + отопление/водоснабжение в одном файле)
сантехнические фитинги уходили в модуль воздуховодов: «Ниппель латунный
резьбовой НР 1" VTr.582.N.0006», «Ниппель переходной 3/4" x 1/2"»,
«Ниппель приварной 1"» получали фейковую цену 40.16₽ (листовой ниппель Ø100
по fallback). Также готовые изделия («Воздухораспределитель для воздуховода
КДР», «Щелевой диффузор … с регулируемым дефлектором») считались как листовые
воздуховоды и «подтверждались» без поиска на сайтах.

### Корень (полный инвентарь, 442 строки)
1. `_resolve_spec_context()` по ИМЕНИ файла («вент») возвращал ventilation для
   ВСЕХ строк, включая сантехнический блок (excel ~367..447).
2. `_PLUMBING_OVERRIDE_RE` не ловил латунь/медь/резьбу/дюйм-кавычку/НР/ВР/PPR/
   «из ПП»/VTr — сантех-фитинги проходили.
3. `nipple`/`duct_straight`/`deflector` попадали в контекстный автопроход без
   требования воздуховодного контекста.
4. `calc_area` для nipple/deflector/tee_round молча подставлял дефолт (Ø100/
   Ø200/0.1м) при отсутствии размеров → любая «дыра» давала «цену».
5. `_DUCT_CONTEXT_RE` ловил подстроки «воздуховод»/«дефлектор» внутри названий
   готовых изделий («…для воздуховода КДР», «диффузор … дефлектором»).

### Фикс — варианты A+C+D
**A (строгий гейт, ductwork_calculator):**
- `_FINISHED_DEVICE_RE`: готовые изделия (воздухораспределители/диффузоры/
  решётки/клапаны/воздухоотводчики) НЕ duct — их цена рыночная (агент ищет).
- `_PLUMBING_OVERRIDE_RE` расширен: латунь/медь/никелир/резьба/приварной/
  под пайку/НР/ВР/ВН/дюйм-кавычка/сгон/футорк/американка/PPR/из ПП/VTr/VT/
  DN/G-резьба; применяется на ВСЕХ уровнях (не только в вент-контексте).

**C (сегментация смешанной спецификации):**
- `ventilation_segments(specs)` / `excel_in_ventilation()`: ядра = строки,
  однозначно распознаваемые без контекста; расширение на соседние (≤12 строк)
  распознающиеся с вент-контекстом, с допуском «дыр» (крестовина).
- runner: `_vent_segments` + `_spec_context_for_row()` — spec_context
  "ventilation" только для строк внутри вент-сегмента.

**D (без молчаливых fallback):**
- `calc_area` для nipple/deflector/tee_round: нет размеров → возврат 0 (→
  calculate_ductwork_row → None → строка уходит обычному агенту).

### Результат (проверено на реальной спецификации)
- duct-строк: 254 → 217; сантех-строк с ценой: 12 → 0.
- Настоящие воздуховоды (явные и контекст-зависимые в вент-части 2..366) НЕ
  потеряны; вне сегментов 2..366 нет ни одной duct-строки.
- 9 «потерянных» = намеренно: 7×КДР (готовые изделия), диффузор с дефлектором,
  сантех-ниппели/тройники/отводы/ПП-фитинги/воздухоотводчики.

### Тесты
`tests/test_ductwork_calculator.py`: +16 (готовые изделия, расширенный
сантех-оверрайд, calc_area без fallback, сегментация).

**130 passed** в затронутых файлах.
